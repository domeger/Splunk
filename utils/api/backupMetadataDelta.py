...
class BackupMetadataDelta(C42Script):
	...
	def start(self):
		...
		if self.args.output and self.args.output.endswith('csv'):
	...
	def __output(self, out, event):
		if self.args.format == 'csv':
			out.write("%s," % event['computerGuid'])
	...
	def calculateDelta(self, deviceGUID):
		...
		if len(delta) > 0:
			with smart_open(self.args.output) as out:
				if self.args.header:
					out.write("computerGuid,eventType,files_MD5Hash,files_fileEventType,files_fileName,files_fileType,files_fullPath,files_lastModified,files_length,timestamp\n")
